...
{
	private static NamespacedKey dataKey = new NamespacedKey(Ouroboros.instance,"JSON_DATA_MAP");
	...
	public static boolean has(String key,PersistentDataHolder dataHolder)
	{
		...
		NamespacedKey NSKey = new NamespacedKey(Ouroboros.instance,key);
		
		// Use faster method first
		if(getCustomDataTags(data).containsKey(NSKey.toString())) { return true; }
	...
	private static void setNative(String key,DType type,Object value,PersistentDataContainer container)
	{
		...
		if(type == DType.STRING && value instanceof String val)
		{
			container.set(NSKey,PersistentDataType.STRING,val);
			return;
		}
		
		if(type == DType.INTEGER && value instanceof Integer)
		{
			int val = (int) value;
		...
		if(type == DType.BOOLEAN && value instanceof Boolean)
		{
		...
		if(type == DType.FLOAT && value instanceof Float)
		{
		...
		if(type == DType.LONG && value instanceof Long)
		{
			long val = (long) value;
		...
		if(type == DType.DOUBLE && value instanceof Double)
		{
		...
		if(type == DType.INTEGER_ARRAY && value instanceof Integer[])
		{
			int[] val = (int[]) value;
		...
		if(type == DType.LONG_ARRAY && value instanceof Long[])
		{
			long[] val = (long[]) value;
		...
		if(type == DType.TAG_CONTAINER && value instanceof PersistentDataContainer val)
		{
			container.set(NSKey,PersistentDataType.TAG_CONTAINER,val);
			return;
		}
		
		if(type == DType.TAG_CONTAINER_ARRAY && value instanceof PersistentDataContainer[] val)
		{
			container.set(NSKey,PersistentDataType.TAG_CONTAINER_ARRAY,val);
			return;
		}
		
		if(type == DType.BYTE && value instanceof Byte)
		{
			byte val = (byte) value;
		...
		if(type == DType.BYTE_ARRAY && value instanceof Byte[])
		{
			byte[] val = (byte[]) value;
		...
		if(type == DType.SHORT && value instanceof Short)
		{
		...
		for(NamespacedKey key : data.getKeys())
		{
			for(DType dType : DType.values())
			{
				...
				switch(dType)
				{
					...
					{						
						for(PersistentDataContainer con : (PersistentDataContainer[]) data.get(key,type))
						{
							printContainer(con);
						}
						break;
